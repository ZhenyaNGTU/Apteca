...

{
   ...
   int num, pric, jj;
    ...
    if (e==NULL)
    {
        ...
        begin=new APT(num,pric,0,jj,dat,nam,ys,znac);
        return begin;
    ...
    else
    {
        ...
        znac=new char[znach.length()];
        strcpy(znac, znach.c_str());
        e->next=new APT(num,pric,0,jj,dat,nam,ys,znac);
        return begin;
    }
...
APT* del(APT* e, int* m, int *sm)
{
    ...
    if(e->next==NULL)
    {
        cout<<"Нельзя удалить единтсвенный элемент списка"<<endl;
        return begin;
    }

    if (e->number==number)
    {
        //buf=new APT(e->number,e->price,e->x,e->j,e->name,e->date, e->ysl,e->znach );
        while(1)
        {
            if(e->next->next==NULL)
                break;
